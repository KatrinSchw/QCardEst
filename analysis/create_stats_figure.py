"""
Create STATS benchmark comparison figure for all classical layers.
Shows QCardEst (Estimation) and QCardCorr (Correction) with PostgreSQL baseline.
"""
...
def parse_filename(filename):
    ...
    for i, part in enumerate(parts):
        if 'stats-statsCards6' in part or 'statsCards6' in part:
            stats_idx = i
            break
    ...
    if stats_idx is not None and stats_idx > 0:
...
def main():
    ...
    stats_csv = costs_dir / "statsCards6.csv"
    
    baseline_postgres = load_baseline_data(stats_csv)
    ...
    plt.tight_layout()
    output_path = figures_dir / "stats_classical_layers_comparison.png"
