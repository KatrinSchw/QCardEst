...
\section{For Presentations}

\textbf{Most Important Functions to Highlight}:

\begin{enumerate}
\item \textbf{\texttt{cardEnv.py:expectedWithType()}} - Shows the Est/Corr difference
\item \textbf{\texttt{ML/GradientQML.py:run()}} - Shows the training loop
\item \textbf{\texttt{cardEnv.py:elvaluateModel()}} - Shows how metrics are computed
\item \textbf{\texttt{ML/models.py:vqc()}} - Shows quantum model architecture
\item \textbf{\texttt{cardEnv.py:listSolutions()}} - Shows final prediction generation
\end{enumerate}
