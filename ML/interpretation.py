...
class ModuloCondense(Interpretation):
    """
    Use a function to reduce the number of values
    """
...
class ModuloMax(ModuloCondense):
    ...
    def condense(self, prediction: torch.Tensor, n: int) -> torch.Tensor:
        result = torch.Tensor([0.0] * n)
        padLength = math.ceil(len(prediction) / n) * n - len(prediction)
        padded = torch.nn.functional.pad(prediction, (0, padLength), mode='constant', value=0)
        result = padded.reshape((-1, n)).max(0)[0]
        return result
